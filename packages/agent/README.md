...
Stateful agent with tool execution and event streaming. Built on `@xcsh/pi-ai`.

## Installation

```bash
npm install @xcsh/pi-agent
```

## Quick Start

```typescript
import { Agent } from "@xcsh/pi-agent";
import { getModel } from "@xcsh/pi-ai";
...
```typescript
import { Agent, streamProxy } from "@xcsh/pi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@xcsh/pi-agent";
