...
This document explains how token/tool streaming is normalized in `@xcsh/pi-ai`, then propagated through `@xcsh/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@xcsh/pi-ai`
