...
Most runtime lookups use `$env` from `@xcsh/pi-utils` (`packages/utils/src/env.ts`).
...
These are consumed via `@xcsh/pi-utils/dirs` and affect where coding-agent stores data.
